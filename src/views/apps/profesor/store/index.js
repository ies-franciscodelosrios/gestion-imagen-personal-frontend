...
import { getUsersByRol, getUserByID, editUser, deleteUserByID, getAllAppointments } from '../../../../services/api'
...
/* ALL PROFESOR */
 ...
 export const getAllData = createAsyncThunk('appProfesors/getAllData', async (params) => {
  ...
  if ((response === null || response.data.data.length <= 0) && params.q == '') {
    Object.assign(response, await getUsersByRol().then(result => { return result }))
  }
...
export const addProfesor = createAsyncThunk('appProfesors/addUserProfesor', async (user, { dispatch, getState }) => {
  ...
  console.log(user)
  const response = await getUsersByRol().then(result => { return result.data.data })
  return response
})
/* UPDATE PROFESOR */
export const updateProfesor = createAsyncThunk('appProfesors/updateUser', async updatedUser => {
  await editUser(updatedUser).then(e => toast.success('Datos Guardados')).catch(e=>toast.error('Error al editar'));
  return updatedUser
})
